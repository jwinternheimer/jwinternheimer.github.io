...
However, **_there is a very long tail that suggests that peopel that do use the feature use it quite frequently_**.
...
Most users don't use Share Now frequently. Around 69% of users in our dataset simply haven't used the feature in the past month, so 0% of their weeks contain updates shared immediately. 

However, **_there is a very long tail that suggests that peopel that do use the feature use it quite frequently_**. Considering that only around 30% of active users used the Share Now feature, the fact that around 19% used in at least 50% of active weeks and 15% used it in _every_ week active is interesting. It seems to be quite a sticky feature for those that use it.
