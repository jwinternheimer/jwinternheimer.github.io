---
date: 2017-11-01T11:38:42-04:00
subtitle: ""
tags: []
type: "post"
...
### TL;DR
 ...
 - The changes have not yet made any detectable effect on Awesome MRR, which still stagnates. 
 
 - Awesome trialists still need time to convert and finish trials.
 
### Background
In September, Buffer experienced a period of relatively low [MRR growth](https://github.com/bufferapp/buffer-analysis/blob/master/deliverables/september_mrr_analysis.md). This was especially impactful for the Awesome plans, which saw a sharp and distinct decrease in growth. 
...
library(dplyr); library(buffer); library(ggplot2); library(CausalImpact)
```
...
ggplot(trials) +
  ...
  labs(x = NULL, y = NULL, title = "Trials Started")
```

Cool, we might be able to detect a nice increase in awesome trial starts here. Let's try to estimate the impact that the change we made on October 13 had on trial starts by building the counterfactual. Remember, we'll use the number of business trials started as a predictor of the number of awesome trials started. 
...
Wow, the results are something! The top panel in the graph shows the counterfactual as a dotted line and blue confidence interval -- this is the estimate of what trials would have been _without an intervention_. The solid black line shows the number of Awesome trials that actually were started.
...
This summary tells us that we've seen an average of 215 awesome trials since the action was taken. The predicted average, based on previous months worth of data, would have been 166. The relative effect was a 30% increase in awesome trial starts -- the 95% confidence interval for this effect size is [24%, 35%]. That's a big effect. 
...
ggplot(mrr) +
  ...
  labs(x = NULL, y = NULL, title = "MRR Values")
```
...
This is an interesting one. Please ignore the `cumulative` panel, as it is not relevant to us here (MRR is already an aggregation and we don't sum it every day). 

The `original` panel shows the counterfactual, as well as the confidence bands, as the dotted line and blue ribbon. The solid black line is the actual MRR amounts we've seen. We can see here that the observed values (the solid black line) are within the bounds of the confidence interval. 

The `pointwise` panel indicates the point estimate of the effect, which ,at its maximum value, was less than \$1000. There doesn't appear to have been any significant effect on MRR yet.
...
The summary confirms that we have not yet seen any significant effect on Awesome MRR.
