---
date: 2017-08-07T14:02:25-04:00
subtitle: ""
type: "post"
tags: []
title: "Predicting Churn with General Linear Models: An Experiment"
---

In a [previous analysis](https://jwinternheimer.github.io/blog/churn-survey-text-analysis/) we found that the most common reason users give for leaving Buffer is that they weren't using, or didn't need, Buffer.

![](https://jwinternheimer.github.io/blog/churn-survey-text-analysis_files/figure-html/unnamed-chunk-17-1.png)

In this analysis, we'll put that theory to the test. We'll create a [natural experiment](https://en.wikipedia.org/wiki/Natural_experiment) in which users exposed to the experimental and control conditions are determined by their own actions. In other words, we'll identify paying customers that stop using Buffer, or use Buffer at a decreasing rate, and compare them to paying customers with more consistent usage.

How can we tell that users have stopped using Buffer? We have to define _use_ in the right context -- for Buffer, the core value prop is saving time by _scheduling social media posts_. Performing this function is the best proxy we have for getting value from Buffer. 
...
## Data collection
The following SQL query returns the number of updates that every Business customer has sent in each week that he or she sent any updates. 
...
# load updates data
biz_users <- readRDS('biz_users.rds')
```

Users that are on Buffer for Business trials are considered to be Business users in this query, so We'll want to filter this dataset to include only business users that have at least one successful charge. The following query will give us the number of successful charges for each user.
...
# load charges data
biz_charges <- readRDS('biz_charges.rds')
```
...
# calculate growth rates for each user
...
Each of these users saw a dramatic increase in the number of updates scheduled at some point. Notice the single user that scheduled 2500 updates in a single week, and _none_ in the next. We'll want to filter users like that out.
...
To test this hypothesis, I'll gather all of those users (there are 726 of them) and monitor their churn rates over the next 60 days. I will compare these to the churn rates of the rest of the users, which will act as a control group.
...
Here is one example of a user with an estimate less than -5 and a p-value less than 0.05.

```{r echo = FALSE}
users_complete %>%
  filter(user_id == '588684576d19b8a25536abbc') %>%
  ggplot(aes(x = update_week, y = update_count)) +
  geom_point() +
  geom_line() +
  labs(x = NULL, y = NULL, title = "Updates Sent By User 588684576d19b8a25536abbc")
```
...
**Why filter out users with less than 4 weeks of updates?**
