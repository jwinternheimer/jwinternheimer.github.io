...
In a [previous analysis](https://jwinternheimer.github.io/blog/churn-survey-text-analysis/) we noted that the most common reason users gave for churning was that they weren't using, or didn't need, Buffer.
...
These are the features we'll analyze:
 ...
 - The web composer. This is what people use to schedule updates from Buffer's web app.
...
Let's go ahead and collect the data.

## Data collection
We'll query the Redshift table `actions_taken` to find the events associated with each user and each feature that we're interested in analyzing. We'll use the `buffer` R package to do this.
...
We now have a dataframe containing 360 thousand unique user-week-feature combinations. There are over 7000 Business customers in this dataset. To compute the percentages properly, we'll need to collect a bit more data, i.e., the total number of active users for each week. We will define _active_ as having _at least 10_ events in the `actions_taken` table in a given week.
...
Now we have the _total_ number of Business customers that were _active_ each week in the past 6 months. Now, we just need to do a bit of data transformation to ge tour datasets ready for analysis.
